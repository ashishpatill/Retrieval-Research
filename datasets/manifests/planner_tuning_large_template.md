...
8. Compare sweep winners (`best_by_mrr`, `best_by_confidence`) and choose defaults.

Current fixture result: `score_rerank_soft` led by MRR, while `route_vote_rerank_strong` led by confidence. Treat that as a hypothesis, not a default change, until a real mixed corpus confirms it.
